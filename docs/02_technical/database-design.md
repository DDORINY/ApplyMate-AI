...
- 사용자 소유 데이터는 `user_id`로 소유자를 식별한다.
- 보호 API는 인증된 사용자의 `user_id` 조건을 반드시 포함한다.
- 상태값은 Enum 또는 제한된 값으로 관리한다.
- 스키마 변경은 Alembic migration으로 관리한다.
- 운영 DB는 승인 없는 직접 변경이나 데이터 삭제를 금지한다.
...
pgvector는 v0.3.x 이후 의미 기반 검색과 분석에 필요할 때 도입합니다. v0.1.2에서는 확장 설치나 벡터 컬럼을 만들지 않습니다.
