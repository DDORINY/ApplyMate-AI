...
- `v1.0.0`은 MVP 릴리스 준비로 완료되었고, 실제 운영 배포와 외부 provider 검증은 별도 운영 단계에서 진행한다.
...
| v0.8.0 | 알림/리마인더 운영화 | 완료 |
| v0.9.0 | E2E/성능/보안 안정화 | 완료 |
| v1.0.0 | MVP 릴리스 | 완료 |

## v1.0.0 완료 방향

- MVP 사용자 흐름과 제외 범위를 확정했다.
- 사용자/운영/릴리스/보안/known limitation 문서를 추가했다.
- E2E smoke를 MVP 주요 화면으로 확장했다.
- 실제 운영 배포와 live provider 검증은 `NEEDS_VERIFICATION`으로 남긴다.
...
- In-app 알림과 사용자 알림 설정을 구현한다.
- 일정 리마인더와 추천 알림 후보, Gmail 후보, 문서 개선 결과를 notification으로 연결한다.
- Email delivery/mock provider/retry 구조를 구현하고 실제 발송은 `NEEDS_VERIFICATION`으로 남긴다.

## v0.9.0 예정 방향

- 브라우저 E2E smoke, 보안 헤더, CORS, rate limit, request id, 감사 로그, readiness, 운영 문서 기반을 완료했다.
- 실제 운영 부하 테스트와 복구 리허설은 v1.0.0 전 검증 항목으로 남긴다.
