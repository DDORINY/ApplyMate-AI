...
- `job_recommendation_reasons`: structured matched/missing/unknown reasons and evidence
- `job_recommendation_feedback`: user feedback such as interested, not interested, hidden, applied, saved for later

Migration:

- 신규 revision: `20260719_2300`
- 이전 revision: `20260719_2200`
...
대시보드 자체는 신규 테이블을 추가하지 않았으며, v0.6.0 기준 현재 migration head는 `20260719_2300`이다.
대시보드는 다음 기존 테이블을 읽기 전용으로 조회해 집계한다.
