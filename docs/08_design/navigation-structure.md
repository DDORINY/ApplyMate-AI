...
| `/jobs` | 채용공고 목록 | 인증 필요 | 완료 | 채용공고 검색, 필터, 페이지네이션, 관심 토글 |
| `/jobs/new` | 채용공고 등록 | 인증 필요 | 완료 | 직접 입력 및 URL 등록 |
| `/jobs/{jobId}` | 채용공고 상세 | 인증 필요 | 완료 | 상세 조회, 수정, 삭제, 원문 URL 확인 |
| `/settings/accounts` | 소셜 계정 연결 | 인증 필요 | 완료 | Google/GitHub 계정 연결, 목록 조회, 해제 |
| `/settings/security` | 계정 보안 | 인증 필요 | 완료 | 이메일 인증, 비밀번호 변경, 세션 관리 |
...
```text
ApplyMate AI
홈 | 회원가입 | 로그인 | 내 계정 | 프로필 | 채용공고 | 계정 연결 | 보안
```
...
- v0.2.0 범위를 벗어난 화면은 아직 네비게이션에 노출하지 않습니다.
...
| 채용공고 목록 | `frontend/src/app/jobs/page.tsx`, `frontend/src/components/jobs/job-list-panel.tsx` |
| 채용공고 등록 | `frontend/src/app/jobs/new/page.tsx`, `frontend/src/components/jobs/job-create-panel.tsx` |
| 채용공고 상세 | `frontend/src/app/jobs/[jobId]/page.tsx`, `frontend/src/components/jobs/job-detail-panel.tsx` |
