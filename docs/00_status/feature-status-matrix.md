# Feature Status Matrix

| 영역 | 기능 | 상태 | 버전 |
| --- | --- | --- | --- |
| 인증 | 이메일 회원가입/로그인 | 완료 | v0.1.1 |
| 인증 | 소셜 로그인 | 완료 | v0.1.3 |
| 계정 | 이메일 인증/비밀번호 복구 | 완료 | v0.1.4 |
| 프로필 | 커리어 프로필 | 완료 | v0.1.2 |
| 공고 | 채용공고 관리 | 완료 | v0.2.0 |
| AI | 채용공고 분석 | 완료 | v0.2.1 |
| AI | 사용자-공고 적합도 분석 | 완료 | v0.2.2 |
| 이력서 | 파일 업로드/관리 | 완료 | v0.3.0 |
| 이력서 | 텍스트 추출 | 완료 | v0.3.1 |
| AI | 이력서 구조화 분석 | 완료 | v0.3.2 |
| 문서 | 지원 문서 생성 | 예정 | v0.3.3 |
| 지원 | 지원 현황 관리 | 예정 | v0.4.0 |
| 일정 | 일정 관리 | 예정 | v0.4.1 |
| 대시보드 | 요약 화면 | 예정 | v0.4.2 |
| 연동 | Google Calendar | 예정 | v0.5.0 |
| 연동 | Gmail | 예정 | v0.5.1 |
| 추천 | 공고 추천 | 예정 | v0.6.0 |
# v0.3.3 Feature Status

| Feature | Status | Verification |
| --- | --- | --- |
| Application document CRUD | Complete | Backend API tests |
| AI draft generation | Complete | Mock provider tests |
| Evidence/source references | Complete | Backend API tests |
| Version save/restore | Complete | Backend API tests |
| Duplicate/archive | Complete | Backend API tests |
| Documents frontend | Complete | lint, type-check, build |
| Real OpenAI document generation | Needs verification | Requires real API key/model |
| Version | Feature | Status |
| --- | --- | --- |
| v0.4.0 | Application tracking | Implemented |
| v0.4.0 | Application status history | Implemented |
| v0.4.0 | Application notes | Implemented |
| v0.4.0 | Document source selectors | Implemented |
